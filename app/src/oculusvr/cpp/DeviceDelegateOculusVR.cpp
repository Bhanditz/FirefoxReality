...
public:
  ...
  static ovrMatrix4f CylinderModelMatrix( const int texWidth, const int texHeight,
                                          ...
                                          const float radius,
                                          const float density, const float s) {
    const ovrMatrix4f scaleMatrix = ovrMatrix4f_CreateScale( s * radius, s * radius * (float)texHeight * VRAPI_PI / density, radius );
  ...
  void Update(const ovrTracking2& aTracking) override {
    ...
    auto t = layer->GetModelTransform(device::Eye::Left).GetTranslation();
    const ovrVector3f translation = { t.x(), t.y(), t.z() + radius};
    const float s = 0.5f * h / (radius * textureHeight * VRAPI_PI / density);

    ovrMatrix4f cylinderTransform = CylinderModelMatrix(textureWidth, textureHeight, translation, rotateYaw, rotatePitch, radius, density, s);

    vrb::Matrix scale = vrb::Matrix::Identity();
    scale.ScaleInPlace(vrb::Vector(radius, radius * textureHeight * VRAPI_PI / density, radius));


    for ( int i = 0; i < VRAPI_FRAME_LAYER_EYE_MAX; i++ ) {
      device::Eye eye = i == 0 ? device::Eye::Left : device::Eye::Right;
      ovrMatrix4f view = ovrMatrixFrom(layer->GetView(eye));
      ovrMatrix4f modelViewMatrix = ovrMatrix4f_Multiply(&view, &cylinderTransform );
      ovrLayer.Textures[i].TexCoordsFromTanAngles = ovrMatrix4f_Inverse( &modelViewMatrix );
      ovrLayer.Textures[i].ColorSwapChain = swapChain;
      ovrLayer.Textures[i].SwapChainIndex = 0;
